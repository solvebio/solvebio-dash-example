...
def current_user():
    if app.auth:
        ...
        return [html.P("(SolveBio Auth not configured)")]
...
@app.callback(
    Output(component_id="graph-mutation-frequencies", component_property="figure"),
...
def mutation_frequencies_total_pop(clicks, genes):
    ...
    return go.Figure(
        data=charts,
        layout=go.Layout(
            title="Proportion of mutation types (total pop.): {}".format(genes[0]),
            barmode="stack",
            height=600,
            xaxis=dict(title="Percentage Frequency"),
            yaxis=dict(title="Cancer Type"),
        ),
    )
...
def mutation_frequency_total_population_charts(gene, **kwargs):
    ...
    facets = dataset.query().facets(
        **{cancer_type_field: {"limit": 1000}, mutation_type_field: {"limit": 1000}}
    )

    records = []
    for f in facets[cancer_type_field]:
        records.append(
            {"cancer_type": f[0], "count": f[1],}
        )
